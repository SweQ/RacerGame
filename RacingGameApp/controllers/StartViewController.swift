...
class StartViewController: UIViewController {
    ...
    func makeAttributedString() {
        ...
        attrString.addAttribute(.foregroundColor, value: UIColor.white, range: (titleText as NSString).range(of: "Racing"))
        attrString.addAttribute(.foregroundColor, value: UIColor.systemRed, range: (titleText as NSString).range(of: "game"))
        attrString.addAttribute(.font, value: UIFont.init(name: "SquareOne-Bold", size: 80.0) ?? UIFont.systemFont(ofSize: 80.0), range: (titleText as NSString).range(of: titleText))
        ...
        if motionManager.isGyroAvailable {
            motionManager.startGyroUpdates(to: .main) { (data, error) in
                ...
                if rotationRateX > 1 || rotationRateY > 1 {
                    self.makeParallaxEffect(x: self.accelerationX, y: self.accelerationY)
                }
            }
        }
    }
    
    func makeParallaxEffect(x: Double, y: Double) {
