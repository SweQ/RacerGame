...
class GameViewController: UIViewController {
    ...
    private var enemiesCarImages = [
        UIImage(named: "enemy1_png"),
        UIImage(named: "enemy2_png"),
        UIImage(named: "enemy3_png"),
        UIImage(named: "enemy4_png")
    ]
    ...
    func showGameOverAlert() {
        let alertController = UIAlertController(title: "Game over".localized() , message: "\("SCORE".localized()): \(score)", preferredStyle: .alert)
    ...
    func startEnemyCreator() {
        enemyTimer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true, block: { (_) in
            ...
            let enemyStartPositionX = CGFloat.random(in: self.roadPartOneImageView.frame.minX...self.roadPartOneImageView.frame.maxX)
    ...
    func moveEnemy(enemy: EnemyCar) {
        ...
        UIView.animate(withDuration: 0.01, delay: 0, options: .curveLinear) {
            enemy.drive(to: self.view.frame.maxY + enemy.model.frame.height)
        } completion: { (_) in
    ...
    private func createPlayerCar() -> PlayerCar? {
        guard let carImage = UIImage(named: "car1_png") else { return nil }
    ...
    private func turnPlayerCar(to direction: TurnDirection) {
        ...
        guard !car.carIsTurning else { return } // now car must not turning
    ...
    private func makeRoad() {
        roadPartOneImageView.image = UIImage(named: "road_png")
        roadPartTwoImageView.image = UIImage(named: "road_png")
        grassPartOneImageView.image = UIImage(named: "grass_png")
        grassPartTwoImageView.image = UIImage(named: "grass_png")
